    ...
    public class UsuarioController : Controller
    {
        ...
        public IActionResult ExcluirUsuario(int id)
        {
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Erro desconhecido." });
                }
            }
            catch (InvalidOperationException ex)
            {
                // Captura a exceção personalizada relacionada à violação de chave estrangeira
                return Json(new { success = false, message = ex.Message });
            }
            catch (KeyNotFoundException ex)
            {
                // Captura o erro quando o usuário não é encontrado
                return Json(new { success = false, message = ex.Message });
            }
            catch (Exception ex)
            {
